...
import type { ApiDocsOptions, DocsTasksApi } from "./types.ts";
...
async function pending(
  ...
  for (const [path, content] of await generate(packages, resolved)) {
...
export const DocsTasks: DocsTasksApi = {
  async apiDocs(
    ...
    const changed = await pending(packages, options);
  ...
  async checkApiDocs(
    ...
    return (await pending(packages, options)).map(([path]) => path);
  },
};
