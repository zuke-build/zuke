...
async function writeApiJson(): Promise<ApiReference> {
...
class ZukeBuild extends Build {
  ...
  syncWebsite = target()
    ...
    .executes(async () => {
      ...
      await DocsTasks.apiDocs(await collectPackageDocs(), docsOptions());
      await writeApiJson();
      ...
      const message =
        `chore: sync docs + api reference for core@${coreVersion}`;
