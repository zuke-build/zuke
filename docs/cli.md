...
| `zuke --help` / `-h`     | Usage.                                                                                                                                                                                                                                                                                                                                                                       |
| `zuke --version` / `-V`  | Print the installed `@zuke/cli` version on the first line, then — inside a project — the Zuke version the build itself runs on, labelled. The first line is bare, so a script can read it. Exits 0 even when the build cannot answer: the reason goes to stderr, and the CLI's own version is still correct. It reports versions, not health — use a build command for that. |
